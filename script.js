...
function queryCurrentWeather( cityName, apiKey, units, addListElement = false ) {
    ...
    $.ajax({
        ...
        $.ajax({
            ...
            // Update center card.
            $( "#content_header" ).text( `${ cwResponse.name } ${ d.getMonth() + 1 }/${ d.getDate() }/${ d.getFullYear() }`);
            ...
            for ( var i = 0; i < 5; i++ ) {
                ...
                var cardTemp = $( `<p>Temperature: ${ Math.round( ocResponse.daily[ i ].temp.day ) } °F<p>` );
            ...
            if ( addListElement ) {
                ...
                $( ".sidebar_city" ).click( function() {
                    queryCurrentWeather( $( this ).text(), APIkey, "imperial", null );
                });
            }
